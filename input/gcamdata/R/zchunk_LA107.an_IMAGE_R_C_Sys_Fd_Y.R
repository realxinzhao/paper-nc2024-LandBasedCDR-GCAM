...
module_aglu_LA107.an_IMAGE_R_C_Sys_Fd_Y <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    # 1. Calculate livestock production by system ----
    ...
    L107.an_Prod_Mt_ctry_C_Y %>%
      ...
      select(-MixedFrac, -PastoralFrac, -value, -IMAGE_region_ID) %>%
      # gather systems
    ...
    L107.an_Prod_Mt_ctry_C_Sys_Y %>%
      ...
      rename(feed = input) %>%
      select(-FeedFrac, -IMAGE_region_ID) ->
    ...
    L107.an_Prod_Mt_ctry_C_Sys_Fd_Y %>%
      ...
      # consumption = [country, commodity, system, feed type, year production] * [country, commodity, system, year IO from IMAGE]:
      mutate(value = value * IOCoef) %>%
      select(-IOCoef, -IMAGE_region_ID)->
