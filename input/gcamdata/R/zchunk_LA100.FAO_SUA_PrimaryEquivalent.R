...
module_aglu_LA100.FAO_SUA_PrimaryEquivalent <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    QCL_PROD_GCAM %>%
      ...
      mutate(Processed = if_else(Prod_diff > 0 & element %in% c("Production", "Processed"),
                                 FBSH_CB[element == "Processed"], 0)) %>%
      mutate(FBSH_CB = FBSH_CB - pmin(Prod_diff, Processed)) %>%
      # When production in FBSH_CB = 0 set to production in QCL so scaling will be consistent
      mutate(FBSH_CB = if_else(FBSH_CB[element == "Production"] == 0 & element == "Production", value, FBSH_CB)) %>%
      # After the above adjustments, re-scale SUA to match production in value
      mutate(value = FBSH_CB * value[element == "Production"]/FBSH_CB[element == "Production"]) %>%
      # fix NA
      mutate(value = if_else(!is.finite(value), FBSH_CB, value)) %>%
      ungroup() %>%
